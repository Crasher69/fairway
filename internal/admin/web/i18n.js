...
const DICTIONARIES = {
  ru: {
    ...
    // --- access to the panel ---
    'Access': 'Доступ',
    'Access to the panel': 'Доступ в панель',
    'Sign in': 'Войти',
    'Sign out': 'Выйти',
    'Password': 'Пароль',
    ...
    'Repeat': 'Ещё раз',
    'At least 8 characters. Everyone signed in with the old password will have to sign in again.': 'Не короче 8 символов. Все, кто вошёл со старым паролем, будут должны войти заново.',
    'Save password': 'Сохранить пароль',
    'Remove password': 'Убрать пароль',
    'Token': 'Токен',
    'The link with the token is printed to the log at every start: <code>admin panel: http://…/?token=…</code>. Opening it saves the token into a cookie, so it is needed only once. A permanent token is set by the <code>-admin-token</code> flag.': 'Ссылка с токеном печатается в лог при каждом запуске: <code>админка: http://…/?token=…</code>. При открытии токен сохраняется в cookie, так что нужен он один раз. Постоянный токен задаётся флагом <code>-admin-token</code>.',
    'password is set': 'пароль задан',
    'no password, token only': 'пароля нет, вход только по токену',
